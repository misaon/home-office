...
- `bun run ui:watch` — rebuild the office UI on change (the page reloads itself).
...
`spikes/*` (verification harnesses) · `docs/` (incl. `plans/` for the current task) · `audit/` (the
audit record). Package scope is
