...
Doporučení: See ADR 006 — a declarative command table that generates both dispatch and help, or a maintained framework. Done in Wave 6: `apps/cli/src/command.ts` defines the `Command` descriptor, `commands/table.ts` holds the 17 of them, `help.ts` is `helpText()` generated by walking that table, and `run.ts` is 19 lines. Verified: `ho help` lists every command, `ho nope` prints `unknown command "nope"` followed by that help, and `ho project` prints `missing project subcommand` followed by just that command's usage.
...
Doporučení: A one-line human summary by default plus a global `--json`. Colour via `yoctocolors`, gated on `process.stdout.isTTY`. Done in Wave 6: `result(human, value)` prints the sentence by default and the payload under `--json`, which `run()` strips from `argv` before dispatch; `doctor` and `resources` print their whole report as JSON under the same flag. Colour comes from `yoctocolors`, gated on `process.stdout.isTTY && NO_COLOR === undefined`. Verified: `ho health` prints `daemon 0.0.0-dev up 326 s since …`, `ho health --json` the object; `ho doctor --json` and `ho resources --json` print the daemon's own payloads.
Odhad: střední
...
Doporučení: One `formatBytes` in `@ho/protocol` (presentation of protocol data, needed by both clients), base 1024, used by all three. Done in Wave 6: `packages/protocol/src/format.ts`, adopted by `doctor.ts`, `resources.ts` and `panels/resources.tsx`. Verified: `ho doctor` and `ho resources` report the same volume total (`1.2 GiB`) in the same units as the panel.
Odhad: triviální
...
**Correction after Wave 6 — measured, and the finding is withdrawn.** Bun restores the terminal itself, so
the leak this finding describes cannot happen. Measured with the repository's pinned Bun (`bun --version` →
`1.4.2`) under a pty harness (`pty.openpty()`, the slave fd held open by the parent the way a shell keeps
its tty open — closing it makes the pty reset its own line discipline and hides the effect entirely):

- control, no Bun involved: a `sh` script that prints a prompt, runs `stty -echo` and is then killed leaves
  the pty with `ECHO` **off** — the leak is real for a process that does not restore;
- `bun` doing the same, through `` $`stty -echo` `` or `Bun.spawnSync(["stty", "-echo"])`, with or without
  iterating `console`, leaves `ECHO` **on** — both when interrupted with `SIGINT` (exit `-2`, i.e. killed by
  the signal, no handler of ours) and when the script exits normally (exit 0);
- and a pty whose `ECHO` was deliberately turned off _before_ Bun started still has it off after Bun exits,
  so Bun restores the state it found rather than unconditionally enabling echo.

Bun's own source says the same: `src/jsc/bindings/c-bindings.cpp` captures `tcgetattr` into
`termios_to_restore_later[fd]` at startup (`:752`), and `bun_restore_stdio()` (`:598`, `tcsetattr` at
`:634`) runs both from `atexit`/`__cxa_atexit` (`:819-825`) and from an `onExitSignal` handler installed
with `sigaction(SIGINT, …)` (`:642`, `:776`) — the comment at `:610` reads "keep the unconditional restore
so the shell prompt comes back cooked."

So Ctrl-C at the prompt cannot leave the user's shell non-echoing on Bun 1.4.2. The `finally` remains
...
### B5.6 – Every bad CLI argument prints a JSON dump of Zod issues

Severita: medium
Kde: `packages/protocol/src/error-message.ts:1-2` at the audited baseline, reached from `apps/cli/src/main.ts:9`; 17 boundary parses in `apps/cli/src` (`secret.ts:37`, `:47`, `task.ts:23`, `:45`, `:68`, `:103`, `agent.ts:52`-`:117`, `session.ts:21`, `chat.ts:24`)
Důkaz: `errorMessage()` returns `error.message`, and a Zod 4 `ZodError`'s `message` is the JSON-stringified
issue array. Observed: `ho secret set bogus` printed a 14-line block starting `ho: [` with `"code":
"invalid_value"`, a `values` array and an escaped `message`; `ho task list --status nope` printed 20 lines
of the same shape.
Dopad: DX: the most common user error — a mistyped enum value — answers with a JSON structure instead of a
sentence, in a CLI whose other errors are one line.
Doporučení: Render `ZodError` with Zod's own `prettifyError()` in `errorMessage()`, which also improves the
daemon's mapped RPC messages since they use the same helper. Done in Wave 6; verified: the same two
commands now print `ho: Invalid option: expected one of "anthropic-oauth-token"|…` and `ho: Invalid option:
expected one of "inbox"|… → at [0]`.
Zdroj: https://zod.dev/error-formatting (read 2026-09-09) for `z.prettifyError()`; `prettifyError` is
exported by the installed zod 4.5.4 (`node_modules/zod/v4/classic/external.d.ts:10`).
Odhad: triviální
...
**Correction after Wave 6 — the second half of the evidence is wrong, and no change ships.** The `chunk`
pattern is not unused: it is what names the JavaScript and CSS that Bun emits for an **HTML** entrypoint.
Removing it and rebuilding produced `chunk-n3g8fk3m.js` and `chunk-t701s5y1.css` (Bun's default chunk
pattern) instead of `index-<hash>.js` / `index-<hash>.css`; restoring it brought `index-nt40nc7v.js` back.
`naming.entry` governs only the HTML file itself. So the config promises nothing it does not deliver, and
the line stays.

Splitting was then measured rather than assumed. Isolated production builds of the two big dependencies:
`pixi.js` **509 679 B**, `react` + `react-dom` + `@tanstack/react-query` **207 517 B**, against a
**1 113 127 B** bundle — so PixiJS is 46 % of it, as the finding says. What the finding did not check is
whether deferring it can help: the office canvas is mounted by `app.tsx` unconditionally and is the first
thing the window shows, so a `React.lazy` boundary around it would delay the primary view rather than the
panels, and add a loading state to a view that has none. Measured over the served bundle in the browser
pane: `responseEnd` 6 ms, the 1 111 KiB script fetched in 5 ms, `domContentLoaded` 73 ms, `loadEventEnd`
74 ms — after which the sprite manifest and ~230 individual PNGs are what the page actually waits on.
Recommendation withdrawn: nothing here is worth a Suspense boundary, and the sprite requests are the
larger target if first paint ever becomes a problem.
Odhad: —
...
Doporučení: Normalise scp-style to `ssh://git@host/owner/repo` before sending, or narrow the client-side test to the accepted schemes and say so in the hint. Done in Wave 6: `repoUrl()` and `repoSourceOf()` in `@ho/protocol` do the normalisation once for both the dialog and `ho project --url`, and the field now names the accepted forms. Verified live — in the office UI `git@github.com:oven-sh/bun.git` inspects as "git repository · default branch main" and fills in name `bun` (before the change the same input failed validation), while `http://github.com/oven-sh/bun.git` answers `repo.url: use an HTTPS or SSH repository URL without embedded credentials`; from the CLI both `--url git@github.com:oven-sh/bun.git` and the `https://` form print `git repository bun on main`.
Odhad: střední

### B6.7 – A tab whose token the daemon rejects reconnects for as long as it is open

Severita: medium
Kde: `packages/ui/src/sync.ts:94-99` at the audited baseline; `packages/daemon/src/server.ts:112`
Důkaz: Found by measurement in Wave 6, not by reading. A browser cannot see the status of a failed
websocket handshake, so `startSync`'s `catch` treated "the daemon rejected this token" exactly like "the
daemon is down" and retried every 2 s forever. One office tab left open across a daemon restart (the token
is minted per launch) wrote **393 `rejected rpc connection` warn lines** into the daemon log in the minutes
it took to run the rest of this wave's checks, and would have gone on indefinitely. Two problems in one:
the UI never tells the user why nothing loads, and the daemon log fills with lines that hide real ones.
Dopad: DX plus diagnostika: a silent, permanently useless tab, and a log that grows at ~2 lines a second
for as long as it stays open.
Doporučení: On a failed handshake ask `GET /health`: if the daemon answers, the token is what it refused —
set the `unauthorized` state the UI already has and wait for a fresh launch URL (`hashchange`) instead of
retrying. Done in Wave 6. Verified: with `ho.token` set to a stale value the page makes **one** attempt
(one new log line), then shows "No daemon token. Open the office with `ho ui`." and adds **0** further
lines over the next 25 s; setting the current `#token=…` on the same tab — no reload — reconnects it to
...
**Done in Wave 6.** `stateVolume` is `` `${volume}-state-${agentId}` `` with `ho.kind: "provider-state"`, and
`gc.ts` prunes `[...VOLUME_KINDS, ...LEGACY_VOLUME_KINDS]` — the legacy constant holds `claude-config`, so
the volumes already on the owner's Docker are still collected. The local identifier is `stateVolume`
throughout `session-run.ts`, so nothing in the code claims "config" for what is provider state.
Odhad: střední
...
Zdroj: https://bun.com/docs/runtime/secrets.md, read 2026-09-08 and again 2026-09-09 ("macOS: Keychain Services · Linux: libsecret … · Windows: Windows Credential Manager", plus "Requires a secret service daemon such as GNOME Keyring or KWallet"; the same page marks the API "new and experimental" — see B25.2). The page documents no error for an absent service, so the fallback below triggers on **any** failure that is not our own timeout rather than on a matched message.

**Done in Wave 6.** The file is `packages/secrets/src/os-credential-store.ts`, the factory
`createOsSecretStore`, the kind `"os"`; `daemon.json` still accepts `"keychain"` and maps it to `"os"` so an
existing config keeps working. `auto` no longer reads `process.platform`: it runs the first operation
against the OS store, keeps it if it answers, and switches permanently to the 0600 file store if it throws
— except on `SecretStoreTimeout` (the 5 s bound from A2.7), which propagates, because "the store is waiting
for the user's Keychain prompt" must not silently downgrade to a file the user did not ask for. The
fallback is reported to the daemon log (`no OS credential store; using the file secret store`).

Verified on both platforms. macOS host: `auto` reads through `Bun.secrets` with no fallback line. Linux
without libsecret (`docker run oven/bun:1.4.2-alpine`): `FELL BACK: libsecret not available`, the value
round-trips, and `secrets.json` contains it — which also records the exact undocumented error text.
Odhad: střední
...
**Done in Wave 6**, and the protocol's own definition backs the number: "A complete interaction cycle
between the Client and Agent, starting with a user message and continuing until the Agent completes its
response", started by one `session/prompt` and ended by one `StopReason`. `TurnState.toolCalls` was the
only reader of that counter, so the field is gone rather than left unread. The separate field for tool
calls was deliberately **not** added: every call is already a `tool_call` runtime event the UI streams and
the office stores, so a second representation would be one more thing to keep true.

Worth stating plainly, because the fix does not make the column uniform: for Claude Code `turns` is
`num_turns` from its own result line (model round-trips inside one `claude -p` run), for ACP it is now the
number of prompts the office sent. Each is that provider's own definition of a turn, and neither is "how
many tools were called", which is what the column reported before. Only Claude Code is given the budget
(`--max-turns`), so nothing enforces a cap on this number for ACP either way.

Verified with the ACP mock harness (`spikes/s6-acp-mock`), driven through the same `RunnerChannel` the
daemon uses, with the mock temporarily emitting three tool calls in one prompt: the audited baseline
reported `result ok=true turns=3`, the new code reports `result ok=true turns=1` for the identical
exchange, with `init`, `permission_request`, `tool_call`, `tool_result` and `result` all still asserted by
the harness (`CHECKS {"init":true,"permission":true,"toolCall":true,"toolResultOk":true,"result":true,"exitCode":0}`).
The mock and the runtime were restored afterwards, so the harness in the tree is unchanged.
Zdroj: https://agentclientprotocol.com/protocol/prompt-turn, read 2026-09-09.
Odhad: střední
...
**Correction after Wave 6 — "log rotation" here means the _containers'_ logs only.** The rotation this
finding credits is Docker's `LogConfig` on the sandboxes. The daemon's own log had none: see B21.4.

### B21.4 – The daemon's own log file has no size bound

Severita: medium
Kde: `packages/daemon/src/logger.ts:7-10` at the audited baseline; the file path comes from `apps/desktop/src/bun/index.ts` through `launch.ts:33`
Důkaz: `pino.destination({ dest: file, mkdir: true, sync: true })` appends for as long as the daemon runs
and across every launch, with nothing that trims or rolls it. B21.3 credited "log rotation", but that is
Docker's `LogConfig` on the sandbox containers — a different file. Measured while verifying B6.7: one
stale browser tab drove the log to **393** lines in minutes, and the same file also carries every session's
records for the whole life of the desktop app.
Dopad: Disk: unbounded growth in the one deployment whose stdout nobody reads. A user who leaves the
desktop app running has no way to notice.
Doporučení: Rotate in the process: keep one previous file and reopen. `pino.destination` exists for exactly
this — "can reopen file paths after a file has been moved away" — so a size check that renames to
`<file>.1` and calls `dest.reopen()` needs no dependency. `pino-roll` was considered and rejected on the
owner's activity rule: 4.0.0, last published 2025-10-06 (11 months ago), and it pulls in `date-fns`.
Done in Wave 6 with an 8 MiB ceiling checked once a minute, so the file costs at most two of those.
Zdroj: https://raw.githubusercontent.com/pinojs/pino/main/docs/help.md and /docs/api.md, read 2026-09-09
("`pino.destination` can reopen closed files (for example, for some log rotation scenarios)");
`pino-roll` figures from the npm registry, read 2026-09-09 (229 554 weekly downloads).
Odhad: triviální
...
Doporučení: Add only `type-fest` (types) and `yoctocolors` (CLI), plus the CI smoke step. `@clack/prompts` was on this list until Wave 6 and is not: the bug it was to fix (B5.4) does not exist.
