...
export function startSteering(
  ...
  const onNote = (event: Extract<StoredEvent, { type: "task.note_added" }>): void => {
    ...
    const from =
      note.author.kind === "human"
        ? "the human who asked for this work"
        : `${author?.name ?? "the boss"}, your boss`;
    const sessionId = sessions.steer(taskId, steerMessage(from, note.text), note.text);
    if (sessionId === null) {
