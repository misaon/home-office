...
- **Worker loop.** A session starts with the task brief, repository facts (branch, how to run checks) and the protocol: work → commit → `ho_report({ status: "review" | "done" | "blocked", summary, branch })`. Workers may `ho_handoff({ toAgentId, brief })` (e.g. to a reviewer or a specialist); the daemon creates the Handoff, emits `handoff.started`, waits for the office animation to complete (or a timeout) before starting the target session — the visual handoff is real, not decorative.
- **Review loop.** `review` status spawns/reuses the reviewer's session on the branch; verdict via `ho_report` (`done` or back to `in_progress` with findings). Max review rounds is a budget.
- **Human gates.** `permission_request` events (if a mode other than bypass is configured) and `ho_ask_human` surface in the chat panel; the agent shows the "question" emotion and waits.
