...
- **Building = floors.** Floor 0 _Lobby_: boss office, reception with the **Board**, mailroom with mailbox, kitchen, relax room, smoking room, toilets, elevator. Floors 1..N: one per project from a **floor template** (desk clusters with monitors, whiteboard showing the project's board summary, kitchenette, toilet, elevator). Templates are generated procedurally in `@ho/sim/templates.ts` (`lobbyTemplate()`, `projectTemplate(id, name, desks)`): floor sprite per cell, wall mask, furniture with footprints, and named **anchors** (`desk`, `boss-desk`, `coffee`, `restroom`, `smoke`, `relax`, `sleep`, `mailbox`, `reception`, `whiteboard`, `elevator`, `wander`). Walls render from a 3×3 autotile chosen by wall neighbours.
...
- **Movement.** Grid A* per floor (walkable mask, standing actors block), 4-direction sprites (west = flipped east), smooth interpolation, reservation of seats/anchors to avoid two agents on one chair. Actors execute **plan steps** (`walk`, `elevator`, `dwell`, `hold`, `emit`); a `walk` to another floor makes the executor insert the elevator ride, so intents are written floor-agnostically. An in-progress handoff (steps up to its `emit`) survives later intents (`pendingHandoff`).
