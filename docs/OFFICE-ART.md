...
The first implementation is a **technical layout prototype**, using simple geometric furniture and the existing placeholder character frames. It does not replace the approved artwork or the live project's floor templates. The six developer desks now use the first art-density sample: generated desk/monitor, textured carpet and empty chairs, with the occupied-chair swap at DEV 3. The workstation source is imported through the native `v2` path, which preserves source pixels and trims only transparent margins; PixiJS applies per-object presentation scales. The logical collision grid remains unchanged except for the DEV 3 desk footprint. Use **Vzorek DEV 3** to zoom in and send Alex to the sample. Alex now has matching four-frame walking clips for north/south/east (west mirrored), neutral idle poses and a four-frame seated typing loop at DEV 3. Sources and prompts are in `assets/reference/alex-v1/README.md`; import with `bun run assets:alex`. Other seated orientations and independent chair/body layers remain pending. Source, generation prompts, frame dimensions and placement are recorded in `assets/reference/workstation-v1/README.md`; use `bun run assets:workstation:native` for the preserved-pixel workstation import.
...
- Geometry follows the approved room arrangement. Main corridor: 4 cells deep; call-booth approach: 4 cells wide; the hot tub has 2 cells clear on both sides. Door thresholds are at least 2 cells wide. Both WC stalls share the restroom north wall (y=33); their fixtures, interaction points and doors move together, leaving no passage behind them.
...
- Door leaves react to actor proximity; the elevator indicator and leaves react to the existing hidden transfer step. The reception backdrop joins the elevator shaft continuously. The kitchen east wall has no doorway to the spa; terrace access is through the north entrance from the main corridor. The kitchen includes the former corridor strip: its tiled interior spans x=49–68, y=21–32, with an open north entrance at x=49–53. Reception flooring is confined to x=10–24, y=22–31; the elevator approaches and right corridor retain the lobby floor. Room bounds drive both floor tiles and the lab surface fill. Fixed south glazing spans x=49–68 at y=33, from the meeting-room wall across the corridor and kitchen to the east wall; it blocks direct terrace access and renders as tall translucent panels in the sorted object layer. Door animation is a visual prototype over permanently walkable thresholds; hinges, locking and final sprites are not implemented.
- Shared routing balances distance, static clearance and turns. Steps beside a wall/furniture cost 4, two cells away 1.5, and three or more cells away 1; every direction change costs 6. A* tracks arrival direction as part of its state, keeping long corridor runs straight instead of producing staircases for small clearance gains. Clearance is cached per grid and invalidated when walkability changes. Narrow doors and wall-adjacent goals remain reachable; both the yellow preview and actual movement use this planner.
...
| Import sizes        | Chosen per finished asset; each object records its native dimensions, pivot, sorting baseline, interaction anchors and collision footprint | The importer accepts the artwork's native output size; visual scale is calibrated per object and is independent of logical grid collision                                  |
