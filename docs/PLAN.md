...
| Medium   | Agent image size                                | The ~900 MB browser layer split into a second image target, if the owner wants it: `browser.enabled` defaults to true, so the saving only reaches installations that turn the browser off, and the change doubles the image matrix and alters a first run's download. Plan recorded in `audit/AUDIT.md` (B24.1) |
...
which two were added and one removed (005), and a hand-written command table over a CLI framework (006).

**Waiting on the owner:** role-aware effort defaults raise what a worker session costs (worker and
reviewer now run at `high`) — the numbers are in `audit/AUDIT.md` (B33.4); and the browser image split
above needs a yes or no.
